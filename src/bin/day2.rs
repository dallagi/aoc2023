...
struct Game {
    id: u32,
    cubes_revealed: Vec<CubesRevealed>,
}
...
impl Game {
    pub fn parse(input: &str) -> Game {
        ...
        let id = parts
            ...
            .parse()
            .unwrap();
        let cubes_revealed = parts
            .next()
            .unwrap()
            .split(";")
            .map(CubesRevealed::parse)
            .collect();
...
impl CubesRevealed {
    pub fn parse(input: &str) -> CubesRevealed {
        let mut result = CubesRevealed::default();
